            ...
            steps{
                script{
                    ...
                    try{
                        ...
                        '''
                        sh(returnStatus: true, script: "cd ${WORKSPACE}/docker/ && docker build -t --env version=${pom.version} helloworld:${pom.version} . && docker run -d -p 11080:8080 --env version=${pom.version} --name helloworld helloworld:${pom.version}")
                    }catch(error){
                        sh(returnStatus: true, script: "cd ${WORKSPACE}/docker/ && docker build -t --env version=${pom.version} helloworld:${pom.version} . && docker run -d -p 11080:8080 --env version=${pom.version} --name helloworld helloworld:${pom.version}")
                    }
                }
